...
* **Structured Plans**: For plan-mode tasks, produce an ordered checklist where every step has an ID, description, optional tool, status, and result.
...
* **Checkpoint Progress**: Update the active plan after every step so a later session can resume from its checklist.
