...
func (t *Grep) Capabilities() tools.CapabilitySet { return tools.CapabilityReadWorkspace }

func (t *Grep) Description() string {
	return "Finds literal or substring matches. Returns file, line, match, and optional context. Path may be a file or directory; use glob or read_file after locating a match."
}

func (t *Grep) Schema() any {
...
func (t *Grep) Execute(ctx context.Context, input any) (*tools.ToolResult, error) {
	...
	if result, index, indexed, err := indexedQuery(ctx, repository.Query{Text: parsed.Pattern, FullText: true, Limit: limit}); indexed {
		...
		return grepResult(matches, truncated), nil
	}
...
func grepResult(matches []TextMatch, truncated bool) *tools.ToolResult {
	msg := "Grep completed"
	if truncated {
		msg = "Grep truncated; narrow path, glob, or pattern"
	}
	return tools.BuildSerializedToolResult(true, msg, GrepOutput{Matches: matches, Truncated: truncated})
}
