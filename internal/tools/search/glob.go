...
// Glob finds files and directories by name pattern within a workspace path.
...
func (t *Glob) Capabilities() tools.CapabilitySet { return tools.CapabilityReadWorkspace }

func (t *Glob) Description() string {
	return "Finds files and directories by name pattern. Use grep or read_file after locating a path."
}

func (t *Glob) Schema() any {
...
func (t *Glob) Execute(ctx context.Context, input any) (*tools.ToolResult, error) {
	// Parse input
		...
		if result, index, indexed, err := indexedQuery(ctx, repository.Query{Text: query, Kind: "file", Exact: true, Limit: tools.MaxSearchResults}); indexed {
			...
			for _, candidate := range result.Candidates {
				...
				if matched, _ := filepath.Match(pattern, value); matched {
					matches = append(matches, GlobMatch{Path: indexedPath(index, candidate), Name: name, Type: "file"})
				}
			}
			return tools.BuildSerializedToolResult(true, "Glob completed", GlobOutput{Matches: matches, Truncated: len(matches) >= tools.MaxSearchResults}), nil
		}
	}
	...
	return tools.BuildSerializedToolResult(true, "Glob completed", output), nil
}
